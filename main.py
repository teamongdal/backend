...
############################################################################################################
############################################################################################################
from fastapi import FastAPI, File, UploadFile, HTTPException, Depends, Query, Form, WebSocket
...
# main.py runs on local machine
# make a online link that can connect to this local server (Azure: app-container? 느낌)
...
async def search_product(
    ...
    try:
        ...
        # ADD original product_code to similar_product_list and pass to get_similar_products
        return {
            "success": True,
...
def get_similar_products(user_id: str, product_code: Optional[str] = Query(None), find_product_code: Optional[List[str]] = Query(None), db: Session = Depends(get_db)):
    ...
    find_product_code = ["blazer_0009", "blazer_0012", "blazer_0015", "blazer_0016"] # [product_id + similar_product_id]
    products = db.query(Product).filter(Product.product_code.in_(find_product_code)).all()
...
def product_like_list(user_id: str, db: Session = Depends(get_db)):
        ...
        "all_product_list": [
            {
                ...
                "similar_products": [h.product_code, products[h.product_code].similar_product_1, products[h.product_code].similar_product_2, products[h.product_code].similar_product_3]
